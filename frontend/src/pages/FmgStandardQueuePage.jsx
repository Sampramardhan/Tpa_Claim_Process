...
function FmgStandardQueuePage() {
  ...
  const loadQueues = useCallback(async () => {
    ...
    setQueueError('');
    try {
      const queue = await getFmgClaimsQueue();
      setClaimQueue(queue || []);
      await hydrateQueueDecisions(queue || []);
    } catch (error) {
      setClaimQueue([]);
      setQueueDecisionState({});
      setQueueError(extractErrorMessage(error, 'Unable to load the FMG standard review queue.'));
    } finally {
  ...
  function syncQueueDecision(claimId, decision) {
    ...
    setQueueDecisionState((current) => ({
      ...current,
      [claimId]: { loading: false, decision: decision || null },
    }));
  }
  ...
  return (
    <PageShell title="Standard Review Queue" eyebrow="FMG Portal">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-ink-900">Standard Review Queue</h3>
            <p className="mt-1 text-sm text-slate-500">
              Claims awaiting initial FMG evaluation and automated rule processing.
            </p>
          </div>
          <button
            type="button"
        ...
        {queueLoading ? (
          ...
          <DataTable
            columns={columns}
            data={claimQueue}
            emptyMessage="No claims pending standard evaluation."
...
export default FmgStandardQueuePage;
